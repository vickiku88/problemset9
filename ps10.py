...
class Hand(object):
    ...
    def containsLetters(self, letters):
        ...
        if set(selfArray).issubset(set(lettersArray)): 
            return True
        return False
    ...
    def __eq__(self, other):
        ...
        selfDict = dict.fromkeys(self, 0)
        for c in self: selfDict[c] += 1
                
        otherDict = dict.fromkeys(other, 0)
        for c in other: otherDict[c] += 1

        for key,value in selfDict.items():
            print key, value
            if key not in otherDict or (key not in otherDict and otherDict[key] != value):
                return False
        return True
